...
function getBirthYear(value: string) {
  ...
  if (!match)
    throw new Error("Введите дату в формате ДД.ММ.ГГГГ, например 25.12.2000.");
    ...
    try {
      ...
      if (mode === "login") await login(values.email, values.password);
        ...
        if (!values.firstName || !values.lastName || !values.birthDate)
          throw new Error("Заполните имя, фамилию и дату рождения.");
        await signUp(values.email, values.password, {
          firstName: values.firstName,
          lastName: values.lastName,
          birthYear: getBirthYear(values.birthDate),
      ...
      >
        ...
        {mode === "register" && (
          <div className="mt-5 grid grid-cols-2 gap-3">
            <Field label={lang === "ru" ? "Имя" : "Ism"}>
              <input {...register("firstName")} />
            </Field>
            <Field label={lang === "ru" ? "Фамилия" : "Familiya"}>
              <input {...register("lastName")} />
            </Field>
            <Field
              label={lang === "ru" ? "Дата рождения" : "Tug‘ilgan sana"}
            ...
            <Field label="Telegram (необязательно)" className="col-span-2">
        ...
        <button
          ...
          className="mt-6 w-full rounded-xl bg-brand-600 py-3 font-bold text-white disabled:opacity-60"
        >
          {mode === "login"
            ? lang === "ru"
              ? "Войти"
        ...
        <button
          ...
          onClick={google}
          disabled={isSubmitting}
          className="mt-3 w-full rounded-xl border py-3 font-bold dark:border-slate-700 disabled:opacity-60"
        >
          G&nbsp;{" "}
...
function Field({
  ...
  return (
    <label className={(className || "") + " block text-sm"}>
      {label}
      <span className="mt-1 block [&_input]:w-full [&_input]:rounded-xl [&_input]:border [&_input]:bg-transparent [&_input]:p-3 dark:[&_input]:border-slate-700">
        {children}
      </span>
    </label>
  );
